...
class Model{
	...
	public function insert($data, $type = 'single'){
		if($type == 'single'){
			...
			/*
			$arrData = array (
					array('name' => 'admin', 'status' => 0, 'ordering' => 0),
					array('name' => 'admin2'),
					array('name' => 'admin3', 'status' => 0, 'ordering' => 0),
			); 
			*/
	...
	// UPDATE
	/*
	UPDATE group SET name = Member, status = 1, ordering = 1
	WHERE id = 1
	*/
	...
	*/
	public function createWhereUpdateSQL($data){
		...
		if(!empty($data)){
			...
			$newWhere = implode(" ", $newWhere);// chuyen arr thanh 1 chuoi
		}
		return $newWhere;
	}
